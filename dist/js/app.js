(() => {
    ...
    function dropdownToggle() {
        document.addEventListener("click", (e => {
            ...
            const isTargetCloseButtons = document.querySelectorAll("[data-filter-close]");
            const mediaQuery = window.matchMedia("(max-width: 768px)");
            if (!isTargetButton && null != e.target.closest("[data-active]")) return;
            let currentTarget;
            if (isTargetButton) {
    ...
    if (descriptionListItems) for (let i = 0; i < descriptionListItems.length; i++) {
    ...
    function changePrice() {
        ...
        radioButtons.forEach((radioButton => {
            const productPrice = radioButton.parentNode.parentNode.nextElementSibling;
            radioButton.addEventListener("change", (function(e) {
                if (radioButton.checked) productPrice.innerHTML = radioButton.value;
            }));
    ...
    if (checkoutForm || cartDropdown) document.addEventListener("click", handle);
    function handle(evt) {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            ...
            const orderSLider = new core(".order-slider", {
